import { W, H, FPS, DURATION } from './time.js';
import { stage, reset } from './stage.js';
import { loadFonts } from './fonts.js';
import { draw, setup } from './timeline.js';
import { finish } from './post.js';
import { soundtrack } from './audio/index.js';

let canvas, ctx, lo, hi, samples, shutter;
...
async function init(opts = {}) {
  ...
  await Promise.all([loadFonts(), setup()]);
}

function renderAt(t) {
  reset(ctx);
  draw(ctx, Math.min(Math.max(t, 0), DURATION - 1e-6));
}
...
function accumulate(t) {
  ...
  for (let k = 0; k < samples; k++) {
    renderAt(t + ((k + 0.5) / samples - 0.5) * (shutter / FPS));
...
async function audio() {
  const bytes = new Uint8Array(await soundtrack());
  let s = '';
  for (let i = 0; i < bytes.length; i += 0x8000) s += String.fromCharCode(...bytes.subarray(i, i + 0x8000));
  return btoa(s);
}

window.reel = { init, frame, audio };
